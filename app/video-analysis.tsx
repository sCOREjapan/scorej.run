...
(f=該当フレーム番号、x/y=左上、w/h=幅高さ)で指定。x/y/w/hは必ず小数第2位までに丸めること
(例:0.23。0.234567のような無駄な桁数は禁止)。根拠位置が明確でない項目には無理に付けず、
本当に指せる箇所がある場合のみ含める（focusとnextStepは最重要なので優先的に付けてよい）。
